...
var getExchangeRates = _dist.getters.getExchangeRates;
var Big = _dist.big.Big,
    maxBN = _dist.big.maxBN,
    minBN = _dist.big.minBN;
...
var LOW_BALANCE_THRESHOLD = new BN(process.env.LOW_BALANCE_THRESHOLD);
...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: async function sendPayment(toAccount, amount) {
      ...
      var payAmount = Web3.utils.isBN(amtWei) ? amtWei : new BN(amtWei);
      var bnBal = new BN(balance);
    ...
    value: function calculateWeiToRefund(wei, connextState) {
      ...
      var ceilingWei = new CurrencyConvertable(CurrencyType.BEI, _bignumber2.default.min(HUB_EXCHANGE_CEILING, CHANNEL_DEPOSIT_MAX), function () {
        return getExchangeRates(connextState);
